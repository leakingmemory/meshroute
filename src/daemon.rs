...
use crate::{
    config, controlproto, endpoint, ethernet, ethertable, eventproto, filedes, handshake, keyex,
...
fn read_control_object<T>(stream: &mut UnixStream) -> Result<T, ()>
where
    T: DeserializeOwned,
{
...
fn write_control(
    ...
    msg_type: controlproto::ControlMsgType,
) -> Result<(), ()> {
    let buf = [0u8; 0];
...
fn handle_control(
    ...
    event_handler: Arc<Mutex<EventHandlerCtx>>,
    config: Arc<Mutex<config::Config>>,
    ctx: &WorkerContextThreadSafe,
) {
    ...
    loop {
        ...
        match cmd {
            ...
            PAIR => {
                ...
                match response {
                    EndpointMessage::PairingResponse => {
                        let pairing_response = match deserialize_from_slice::<PairingResponse>(
                        ...
                        let mut master_pubkey_sha256 = [0u8; 32];
                        {
                            let mut config = config.lock().unwrap();
                            {
                                let digest = sha2::Sha256::digest(
                                    config.master_key.clone().unwrap().public_key.as_slice(),
                                )
                                .as_slice()
                                .to_vec();
                            ...
                            config.pairing_requests.retain(|pairing_request| {
                                ...
                                if pairing_request.master_pubkey_sha256.len()
                                    != endpoint.endpoint_security.master_pubkey_sha256.len()
                                {
                                    return true;
                                }
                                for i in 0..endpoint.endpoint_security.master_pubkey_sha256.len() {
                                    if endpoint.endpoint_security.master_pubkey_sha256[i]
                                        != pairing_request.master_pubkey_sha256[i]
                                    {
                                        return true;
                                    }
                                }
                        ...
                        match write_control_object(
                            &mut stream,
                            controlproto::ControlMsgType::PairingResponse,
                            &pair_result,
                        ) {
                            Ok(_) => {}
                            Err(_) => {
            ...
            LIST_PAIRING_REQUESTS => {
                ...
                match write_control_object(
                    &mut stream,
                    controlproto::ControlMsgType::PairingRequestList,
                    &pairing_requests,
                ) {
                    Ok(_) => {}
                    Err(_) => {
            ...
            LIST_PAIRED_ENDPOINTS => {
                ...
                match write_control_object(
                    &mut stream,
                    controlproto::ControlMsgType::PairedList,
                    &paired_endpoints,
                ) {
                    Ok(_) => {}
                    Err(_) => {
            ...
            ACCEPT_PAIRING => {
                ...
                let mut master_pubkey_sha256 = [0u8; 32];
                {
                    ...
                    config.pairing_requests.retain(|pairing_request| {
                        ...
                        if pairing_request.master_pubkey_sha256.len()
                            != accept_pairing_ctrl.key_hash.len()
                        {
                            return true;
                        }
                        for i in 0..accept_pairing_ctrl.key_hash.len() {
                            if accept_pairing_ctrl.key_hash[i]
                                != pairing_request.master_pubkey_sha256[i]
                            {
                                return true;
                            }
            ...
            INFO_REQUEST => {
                ...
                let mut node_expiry: i64 = 0;
                {
                    ...
                    master_pubkey_sha256 = match &config.master_key {
                        Some(master_key) => sha2::Sha256::digest(master_key.public_key.as_slice())
                            .as_slice()
                            .to_vec(),
                        None => Vec::new(),
                    };
                    node_pubkey_sha256 = match &config.node_key {
                        Some(node_key) => {
                            ...
                            sha2::Sha256::digest(node_key.key.public_key.as_slice())
                                .as_slice()
                                .to_vec()
                        }
                        None => Vec::new(),
                ...
                match write_control_object(
                    &mut stream,
                    controlproto::ControlMsgType::InfoResponse,
                    &info,
                ) {
                    Ok(_) => {}
                    Err(_) => {
            ...
            LIST_LINKS => {
                ...
                let links_resp = controlproto::LinksList { links };
                match write_control_object(
                    &mut stream,
                    controlproto::ControlMsgType::LinksList,
                    &links_resp,
                ) {
                    Ok(_) => {}
                    Err(_) => {
            ...
            ADD_LINK => {
                ...
                };
                {
                    ...
                    config
                        .links
                        .retain(|l| !l.eq_ignore_ascii_case(add_link.name.as_str()));
...
impl EthernetHandlerCtx {
    pub fn new(
        event_writer: PipeWriter,
        uplinks: Arc<Mutex<Vec<Arc<Mutex<uplink::Uplink>>>>>,
        uplink_mactables: Arc<Mutex<HashMap<Vec<u8>, Arc<Mutex<ethertable::MacTable>>>>>,
        masterkey_hash: Vec<u8>,
    ) -> Self {
        Self {
...
pub fn handle_ethernet_frame(ctx: &mut EthernetHandlerCtx) -> Result<(), ()> {
    ...
    let mut uplink_addr: Vec<u8> = Vec::new();
    {
        ...
        for (key, mactable) in uplink_mactables {
            ...
            if uplink_addr.is_empty()
                && (&ctx.frame.dst_mac).is_individual()
                && mactable.has_entry(&ctx.frame.dst_mac)
            {
                uplink_addr = key;
            }
        }
...
impl WorkerContext {
    pub fn run_worker(&mut self) -> libc::c_int {
        ...
        let uplink_mac_tables: Arc<Mutex<HashMap<Vec<u8>, Arc<Mutex<ethertable::MacTable>>>>> =
            Arc::new(Mutex::new(HashMap::new()));
        let uplink_serial_window: Arc<
        ...
        let masterkey_hash;
        {
            ...
            masterkey_hash = sha2::Sha256::digest(masterkey_pub.as_slice())
                .as_slice()
                .to_vec();
        }
            ...
            if let Some(listen_socket) = listen_socket {
                ...
                let _network_listener = thread::spawn(move || {
                    ...
                    for connection in listen_socket.incoming() {
                        ...
                        client_threads.retain(move |thread| !thread.is_finished());
                        ...
                        let network_handler = thread::spawn(move || {
                            ...
                            let mut endpoint =
                                endpoint::Endpoint::new(connection, endpoint_security);
                            ...
                            loop {
                                ...
                                };
                                {
                                    ...
                                    for paired in &config.paired_endpoints {
                                        let mut matching = paired.master_pubkey_sha256.len()
                                            == endpoint
                                                .endpoint_security
                                                .master_pubkey_sha256
                                                .len();
                                        if matching {
                                            for i in 0..endpoint
                                                ...
                                                .len()
                                            {
                                                if endpoint.endpoint_security.master_pubkey_sha256
                                                    [i]
                                                    != paired.master_pubkey_sha256[i]
                                                {
                                                    matching = false;
                                                    break;
                                                }
                                ...
                                match req_type {
                                    EndpointMessage::PairingRequest => {
                                        let pairing_request = match deserialize_from_slice::<
                                            endpoint::PairingRequest,
                                        >(
                                            &req_vec
                                        ) {
                                            Ok(r) => r,
...
pub fn run_daemon_w(restart_me_writer: &mut PipeWriter, opts: &opts::Opts, name: &str) -> ExitCode {
    ...
    let config = Arc::new(Mutex::new(
        match config::Config::from_file(config_file_name.as_str()) {
            Ok(config) => config,
    ...
    ));
    {
        ...
        if let Some(ref nodekey) = config.node_key {
            ...
            {
                let verifying_key =
                    rsa::pkcs1v15::VerifyingKey::<sha2::Sha512>::new_unprefixed(public_key);
    ...
    println!(
        "Configured ethernet device {}",
        unsafe { CStr::from_ptr(ifreq.ifr_name.as_ptr() as *const libc::c_char) }
            .to_str()
            .unwrap()
    );
    ...
    let ctx: WorkerContextThreadSafe;
    {
        let event_reader;
        {
            ...
            ctx = WorkerContextThreadSafe {
                restart_me_writer: Arc::new(Mutex::new(restart_me_writer.try_clone().unwrap())),
                forkedworker: Arc::new(Mutex::new(forkedworker)),
                ctx: Arc::new(Mutex::new(WorkerContext {
                    name: name.to_string(),
                    config: config.clone(),
                    config_filename: config_file_name.clone(),
                    event_writer,
                    tap_dev,
    ...
    for stream in control_listener.incoming() {
        ...
        match stream {
            Ok(stream) => {
                ...
                let config = config.clone();
                let ctx = ctx.clone();
                control_clients.push(Some(thread::spawn(move || {
                    handle_control(
